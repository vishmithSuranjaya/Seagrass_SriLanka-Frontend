          ...
          >
            ...
            {payment.orders && payment.orders.map((order) => (
              <div key={order.id} className="ml-0 sm:ml-4 mt-4 border-t pt-3">
                <h4 className="font-semibold text-gray-600 mb-3">Order ID: {order.order_id}</h4>
                  ...
                  {order.items && order.items.map((item) => (
                    <div key={item.id} className="flex items-center justify-between py-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-gray-700">{item.product?.title || `Product ${item.product_id}`}</p>
                          <p className="text-sm text-gray-500 font-semibold">Unit Price: Rs. {item.product?.price || 0}</p>
                        </div>
                      </div>
                      <div className="text-gray-800 font-medium font-semibold">
                        Rs. {(item.product?.price || 0) * item.quantity} ({item.quantity} × {item.product?.price || 0})
                      </div>
                    </div>
                  ))}
                </div>
